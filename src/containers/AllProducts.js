...
class AllProducts extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:5003/user/all-products", {
        headers: {
    ...
    axios
      .get("http://localhost:5003/user/get-cart", {
        headers: {
          Authorization: "bearer " + this.props.token,
        },
      })
  ...
  addToCart = (id) => {
    ...
    axios
      .post(`http://localhost:5003/user/add-cart/${id}`, data, {
        headers: {
          Authorization: "bearer " + this.props.token,
        },
      })
  ...
  deleteHandler = (id) => {
    let data = null;
    axios.post(`http://localhost:5003/user/delete-cart/${id}`, data, {
      headers: {
        Authorization: "bearer " + this.props.token,
      },
    });
  ...
  render() {
    ...
    return (
      <AllProds>
        ...
        <Korz>
          <StyledBadge
            onClick={(event) => this.showCart(event.currentTarget)}
            aria-label="cart"
          >
        ...
        {this.state.showeDetails ? (
          <div className={this.state.opac}>{post} </div>
        ) : null}
      </AllProds>
    );
  }
