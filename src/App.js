...
class App extends Component {
  ...
  signupHandler = (event, authData) => {
    ...
    axios
      .put("http://localhost:5003/auth/signup", Data)
      .then((res) => {
        if (res.status === 422) {
          console.log("Error!");
        }
  ...
  loginHandler = (event, logData) => {
    ...
    axios
      .post("http://localhost:5003/auth/login", Data)
      .then((res) => {
        if (res.status === 422) {
          console.log("Error!");
        }
    ...
    return (
      <div className="main">
        <BrowserRouter>
          {red}
          <NavLinks
            langHandler={(e) => this.langHandler(e)}
          ...
          <Switch>
            <Route
              ...
              exact
              render={(props) => (
                <LazyAllProds {...props} token={this.state.token} />
              )}
            ></Route>
            {this.state.isAuth ? (
              <Fragment>
                <Route
                  ...
                  exact
                  render={(props) => (
                    <MyProducts {...props} token={this.state.token} />
                  )}
                ></Route>
                <Route path="/search" exact component={Search}></Route>
                <Route
                  path="/Myproducts/:prodId"
                  exact
                  render={(props) => (
                    <MyProducts {...props} token={this.state.token} />
                  )}
